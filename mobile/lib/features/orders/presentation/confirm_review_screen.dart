...
class ConfirmReviewScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ConfirmReviewScreen> createState() => _ConfirmReviewScreenState();
}
...
class _ConfirmReviewScreenState extends ConsumerState<ConfirmReviewScreen> {
  ...
  Future<void> _submit(String counterpartyId) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(widget.reviewOnly ? 'Thanks for the review' : 'Payment released — thank you!'),
        ),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(widget.reviewOnly ? 'Leave a review' : 'Confirm & review')),
      body: AsyncValueView(
        value: order,
        ...
        data: (o) {
          ...
          return ListView(
            ...
            children: [
              ...
              FilledButton(
                onPressed: _busy ? null : () => _submit(o.counterparty?.id ?? ''),
                child: _busy
                    ? const SizedBox(
                        height: 20,
                        width: 20,
                        child: CircularProgressIndicator(strokeWidth: 2))
                    : Text(widget.reviewOnly
                        ? 'Submit review'
