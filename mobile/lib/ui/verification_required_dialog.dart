...
import '../features/auth/presentation/verify_otp_screen.dart' show isVerifyScreenActive;
...
///
/// Two guards keep this from nagging: it never shows while [VerifyOtpScreen]
/// is already the active screen — the whole point of Continue is to get
/// there, so popping it up again there (even mid-way, e.g. right after
/// confirming email while phone is still outstanding) is both pointless and
/// was happening because background polling (the unread chat/notification
/// badge counts, unscoped to any particular screen) hits the same gated
/// check every 15-20s regardless of where the user is — and it won't
/// re-show within [_cooldown] of the last dismissal for the same reason
/// (Cancel felt broken when the very next poll popped it right back up).
Future<void> showVerificationRequiredDialog(BuildContext context) async {
  if (_showing || isVerifyScreenActive) return;
